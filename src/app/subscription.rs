...
use crate::app::constants::{MUSIC_POLL_INTERVAL_MS, ZMQ_POLL_INTERVAL_MS};
...
pub fn subscriptions(state: &State) -> iced::Subscription<Message> {
    ...
    let mut subs = vec![
        ...
        time::every(Duration::from_millis(ZMQ_POLL_INTERVAL_MS)).map(|_| Message::ZmqPollTick),
    ];
    if state.music.is_some() {
        subs.push(time::every(Duration::from_millis(MUSIC_POLL_INTERVAL_MS)).map(|_| Message::MusicPollTick));
    }
